...
/// Get a Alias from the database
#[instrument(skip(db))]
pub async fn get(
    db: impl PgExecutor<'_>,
    auth: &AuthInfo,
    id: AliasId,
) -> Result<Alias, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let object = query_file_as!(
        Alias,
        "src/models/alias/select_one.sql",
        id.as_uuid(),
...
/// Get a populated Alias from the database
#[instrument(skip(db))]
pub async fn get_populated(
    db: impl PgExecutor<'_>,
    auth: &AuthInfo,
    id: AliasId,
) -> Result<AliasPopulatedGetResult, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let object = query_file_as!(
        AliasPopulatedGetResult,
        "src/models/alias/select_one_populated.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids
    )
...
) -> Result<Vec<AliasListResult>, error_stack::Report<Error>> {
...
pub async fn create(
    db: &mut PgConnection,
    auth: &AuthInfo,
    payload: AliasCreatePayload,
) -> Result<AliasCreateResult, error_stack::Report<Error>> {
...
/// Create a new Alias in the database, allowing the ID to be explicitly specified
...
pub async fn update(
    ...
    let result = query_file_scalar!(
        "src/models/alias/update.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids,
        &payload.name as _,
        &payload.random_order as _,
    )
...
pub async fn lookup_object_permissions(
    db: impl PgExecutor<'_>,
    auth: &AuthInfo,
    #[allow(unused_variables)] id: AliasId,
) -> Result<Option<ObjectPermission>, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file_scalar!(
        "src/models/alias/lookup_object_permissions.sql",
        auth.organization_id.as_uuid(),
        &actor_ids,
    )
    .fetch_one(db)
