...
/// Get a ProviderApiKey from the database
#[instrument(skip(db))]
pub async fn get(
    db: impl PgExecutor<'_>,
    auth: &AuthInfo,
    id: ProviderApiKeyId,
) -> Result<ProviderApiKey, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let object = query_file_as!(
        ProviderApiKey,
        "src/models/provider_api_key/select_one.sql",
        id.as_uuid(),
    ...
    .ok_or(Error::NotFound("ProviderApiKey"))?;
...
) -> Result<Vec<ProviderApiKeyListResult>, error_stack::Report<Error>> {
...
/// Create a new ProviderApiKey in the database.
pub async fn create(
    db: &mut PgConnection,
    auth: &AuthInfo,
    payload: ProviderApiKeyCreatePayload,
) -> Result<ProviderApiKeyCreateResult, error_stack::Report<Error>> {
...
/// Create a new ProviderApiKey in the database, allowing the ID to be explicitly specified
...
pub async fn create_raw(
    ...
    let result = query_file_as!(
        ProviderApiKey,
        "src/models/provider_api_key/insert.sql",
        id.as_uuid(),
...
pub async fn update(
    ...
    auth: &AuthInfo,
    id: ProviderApiKeyId,
    payload: ProviderApiKeyUpdatePayload,
) -> Result<bool, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file_scalar!(
        "src/models/provider_api_key/update.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids,
        &payload.name as _,
...
pub async fn delete(
    ...
    let result = query_file!(
        "src/models/provider_api_key/delete.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids
    )
...
pub async fn lookup_object_permissions(
    db: impl PgExecutor<'_>,
    auth: &AuthInfo,
    #[allow(unused_variables)] id: ProviderApiKeyId,
) -> Result<Option<ObjectPermission>, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file_scalar!(
        "src/models/provider_api_key/lookup_object_permissions.sql",
        auth.organization_id.as_uuid(),
        &actor_ids,
    )
    .fetch_one(db)
